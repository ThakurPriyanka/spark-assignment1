...
class Sales {
  ...
  def getSales(sc: SparkContext): Unit = {
    val file1 = sc.textFile("folder/folder3/file1")
    val file1Data = file1.map(line => (line.split('#')(0),line.split('#').toList))

    val file2 = sc.textFile("folder/folder3/file2")
    ...
    val sumYear = finalYear.map(x=> (x._2.map(t=>t._2._1(4)),x._1,x._2.map(y=> y._2._2(2).toInt).sum))


    val finalMonth = data.flatMap(x=> x._2.groupBy(y=>new DateTime((y._2._2(0)).toLong * 1000).toDateTime.toString("yyyy/MM")))

    val sumMonth = finalMonth.map(x=> (x._2.map(t=>t._2._1(4)),x._1,x._2.map(y=> y._2._2(2).toInt).sum))

    val finalDate = data.flatMap(x=> x._2.groupBy(y=>new DateTime((y._2._2(0)).toLong * 1000).toDateTime.toString("yyyy/MM/dd")))

    val sumDate = finalDate.map(x=> (x._2.map(t=>t._2._1(4)),x._1,x._2.map(y=> y._2._2(2).toInt).sum))
